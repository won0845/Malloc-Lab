...
static void *coalesce(void *bp)
{
    ...
    /*
    next_fit은 이전 검색이 종료된 지점에서 검색을 시작하는데 병합할때 위치를 업데이트 하는 이유
    1. 메모리 효율성증대 : 메모리 병합시 큰 블록을 합쳐 블록이 할당될 확률이 높아 바로 변경한다.
    2. 병합으로 인해 메모리 구조가 변경되어 next_fit이 가르키던 위치가 유효하지 않을 수 있고
    새로운 가용블럭이 생겼을 경우 next_fit의 위치를 업데이트하여 다음 할당시 새로운 가용 블럭을 바로 활용할 수 있게 한다.
    */
    return bp;
}
